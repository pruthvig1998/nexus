...
| Ticker | Cap | Sector | Sharpe | CAGR | Max DD | Win Rate | Longs | Shorts |
|:--|:--|:--|--:|--:|--:|--:|--:|--:|
| AAPL | Large | Technology | 2.81 | 2.7% | 0.9% | 92% | 999 | 288 |
| MSFT | Large | Technology | 2.57 | 2.1% | 1.3% | 90% | 1,088 | 286 |
| NVDA | Large | Technology | 2.80 | 3.9% | 1.8% | 90% | 1,027 | 244 |
| GOOGL | Large | Technology | 2.85 | 2.5% | 0.8% | 91% | 1,002 | 268 |
| JPM | Large | Financials | 2.52 | 2.5% | 1.3% | 90% | 1,105 | 352 |
| V | Large | Financials | 2.67 | 2.2% | 0.8% | 91% | 1,039 | 206 |
| JNJ | Large | Healthcare | 2.12 | 1.0% | 0.8% | 90% | 950 | 229 |
| UNH | Large | Healthcare | 2.98 | 2.2% | 0.8% | 92% | 1,082 | 211 |
| XOM | Large | Energy | 2.11 | 1.6% | 1.4% | 89% | 1,092 | 240 |
| PG | Large | Consumer Staples | 2.59 | 1.4% | 0.8% | 89% | 1,137 | 285 |
| DECK | Mid | Consumer Disc. | **3.48** | **4.3%** | 1.1% | **93%** | 1,008 | 269 |
| WIX | Mid | Technology | 2.87 | 4.0% | 1.6% | 92% | 473 | 132 |
| CRUS | Mid | Semiconductors | 2.89 | 3.9% | 2.0% | 92% | 869 | 275 |
| SAIA | Mid | Industrial | **3.46** | **4.3%** | 0.9% | **94%** | 910 | 250 |
| FIX | Mid | Industrial | 2.92 | 3.6% | 1.6% | 92% | 994 | 269 |
| MGRC | Mid | Financials | 3.14 | 3.0% | 1.2% | 93% | 1,006 | 237 |
| PRDO | Small | Education | **3.26** | **4.2%** | 1.0% | **94%** | 804 | 243 |
...
```
                ...
                │            │            │
         ┌──────▼──────┐ ┌──▼───┐ ┌──────▼──────┐
         │   engine    │ │config│ │  backtest   │
         │  trade loop │ │ .env │ │  simulator  │
         └──┬───┬───┬──┘ └──────┘ └─────────────┘
            │   │   │
      ┌─────┘   │   └─────┐
      │         │         │
...
The AI Fundamental strategy uses **Claude Opus 4.6** (Anthropic) to analyze:
...
### Kelly + ATR Sizing Formula

```python
# Kelly criterion (edge / odds)
kelly_fraction = (win_rate * avg_win - (1 - win_rate) * avg_loss) / avg_win
kelly_capped   = min(kelly_fraction, 0.25)   # never risk > 25%

# ATR-based risk budget
atr_stop       = 2.0 * ATR(14)
risk_per_share = atr_stop
shares         = (portfolio_value * kelly_capped * risk_pct) / risk_per_share

# Final position size
position_size  = min(shares, max_position_cap)
```

<br/>

### Short-Specific Safeguards
...
```bash
# Backtest 2 years on 6 tickers
python -m nexus backtest -t AAPL -t MSFT -t NVDA -t GOOGL -t JPM -t AMD --years 2

# Backtest with custom output
python -m nexus backtest -t AAPL -t NVDA --years 1 -o results/my_backtest.json
```

### Monitor

```bash
# Print open positions and daily P&L
python -m nexus status

# Show last 20 closed trades
python -m nexus trades --limit 20
...
NEXUS uses a pluggable broker interface. To add a new broker, subclass `BaseBroker`:
...
```python
from nexus.strategy import BaseStrategy, Signal
...
class PairsStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: dict) -> list[Signal]:
        spread = self.calculate_spread(data["AAPL"], data["MSFT"])
        z_score = self.z_score(spread, window=60)

        signals = []
        if z_score > 2.0:
            signals.append(Signal("AAPL", "short", score=8, strategy=self.name))
            signals.append(Signal("MSFT", "long",  score=8, strategy=self.name))
        elif z_score < -2.0:
            signals.append(Signal("AAPL", "long",  score=8, strategy=self.name))
            signals.append(Signal("MSFT", "short", score=8, strategy=self.name))
        return signals
```
...
# Lint
ruff check nexus/
ruff format nexus/ --check
```

### GitHub Actions CI

Every push triggers:
- **pytest** — full unit + integration test suite
- **ruff** — linting + format check
- **Weekly backtest** — automated 2-year backtest, results uploaded as artifact
